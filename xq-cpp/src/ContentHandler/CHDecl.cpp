...
/** \file CHDecl.cpp
 * \brief Implementation of CHDecl
 */
...
namespace xqML
{
  void CHDecl::startDocument(const string & encoding)
  {
    putReservedSymbol(ch_context.out, 0x1E);
    putReservedSymbol(ch_context.out, 0x00); // Format
    putReservedSymbol(ch_context.out, 0x02); // Format
    putReservedSymbol(ch_context.out, 0x04)<<encoding; // Revision

    ch_context.ch_state = ch_context.state_box.getState(Prolog);
  }

  void CHDecl::startDocument()
  {
    startDocument("");
  }
